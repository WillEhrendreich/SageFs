  ...
  testSequenced <| testList "[Integration] MCP Server Integration tests" [
    ...
    <| fun _ ->
      task {
        ...
        Expect.isGreaterThan (SageFs.EventTracking.getEventCount ctx.Store ctx.SessionMap.["test"] |> fun t -> t.GetAwaiter().GetResult()) 0 "Should track events"
    ...
    <| fun _ ->
      task {
        ...
        do! SageFs.EventTracking.trackInput ctx.Store ctx.SessionMap.["test"] Console "let userValue = 42"
    ...
    <| fun _ ->
      task {
        ...
        Expect.isGreaterThan (SageFs.EventTracking.getEventCount ctx.Store ctx.SessionMap.["test"] |> fun t -> t.GetAwaiter().GetResult()) 0 "Should track error event"
    ...
    <| fun _ ->
      task {
        ...
        Expect.isGreaterThan (SageFs.EventTracking.getEventCount ctx.Store ctx.SessionMap.["test"] |> fun t -> t.GetAwaiter().GetResult()) 0 "Should track execution"
