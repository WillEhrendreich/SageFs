...
let starColors    = [| Color.Gold; Color.SkyBlue; Color.Pink; Color.LightGreen; Color.Orange |]
...
let updatePlayer (dt: float32) (x: float32) =
  let dx =
    (if Raylib.IsKeyDown(KeyboardKey.Left)  || Raylib.IsKeyDown(KeyboardKey.A) then -1.0f else 0.0f) +
    (if Raylib.IsKeyDown(KeyboardKey.Right) || Raylib.IsKeyDown(KeyboardKey.D) then  1.0f else 0.0f)
...
while not (Raylib.WindowShouldClose()) do
  ...
  state <-
    if state.GameOver then
      if Raylib.IsKeyPressed(KeyboardKey.Space) then initState() else state
    else
