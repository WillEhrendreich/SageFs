...
/// Moved to `SessionPathValidation.validateSessionCreateRequest` (its own
/// small module that compiles BEFORE this file, so the `create_session` MCP
/// tool in Mcp.fs — which also compiles before McpServer.fs — can call the
/// SAME validator this route uses, without bloating McpAdapter.fs's
/// formatting grab-bag past its file-size budget). Re-exported here under
/// its original name so this route body and existing tests
/// (`open SageFs.Server.McpServer`) don't need to change (sagefs-roast.md
/// Finding #1/#13: one rule, one implementation).
let validateSessionCreateRequest = SageFs.SessionPathValidation.validateSessionCreateRequest
