...
RETIRED — this is not a registered MCP tool. There is no `load_fsharp_script`
in `tools/list`, and a call to it is refused. To run a script, read the file and
send its contents through send_fsharp_code, or send the file's statements
directly. This formatter is kept only as an internal compatibility path.

WHEN TO USE vs send_fsharp_code:
- send_fsharp_code handles both cases: a complete .fsx on disk, or small code snippets.
