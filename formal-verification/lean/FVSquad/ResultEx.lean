...
/-- Helper: go acc xs = .ok ws → ws.length = acc.length + xs.length -/
private theorem resSequence_go_length {α ε : Type} :
    ∀ (acc : List α) (xs : List (Except ε α)) (ws : List α),
    resSequence.go acc xs = .ok ws → ws.length = acc.length + xs.length := by
  intro acc xs
  induction xs generalizing acc with
  | nil =>
    intro ws hw
    unfold resSequence.go at hw
    simp only [Except.ok.injEq] at hw
    rw [← hw]; simp [List.length_reverse]
  | cons hd tl ih =>
    intro ws hw
    unfold resSequence.go at hw
    cases hd with
    | error e => exact absurd hw (by simp)
    | ok v =>
      have hlen := ih (v :: acc) ws hw
...
theorem resSequence_length {α ε : Type} (xs : List (Except ε α)) (vs : List α)
  ...
  have := resSequence_go_length [] xs vs (by simpa [resSequence] using h)
  simpa using this
...
/-- Helper: for any initial acc, the total length is preserved. -/
private theorem resPartition_foldl_length {α ε : Type}
    (xs : List (Except ε α)) (a0 : List α) (e0 : List ε) :
    let (oks, errs) := xs.foldl (fun acc r =>
      match r with
      | .ok v  => (acc.1 ++ [v], acc.2)
      | .error e => (acc.1, acc.2 ++ [e])) (a0, e0)
    oks.length + errs.length = a0.length + e0.length + xs.length := by
  induction xs generalizing a0 e0 with
  | nil => simp
  | cons hd tl ih =>
...
theorem resPartition_length {α ε : Type} (xs : List (Except ε α)) :
  ...
  simp only [resPartition]
  have := resPartition_foldl_length xs [] []
  simp at this
  exact this
