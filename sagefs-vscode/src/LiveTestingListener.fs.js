...
import { option_type, record_type, array_type, obj_type, lambda_type, unit_type, list_type } from "./fable_modules/fable-library-js.4.29.0/Reflection.js";
...
export function parseTestResult(entry) {
    let f_2, f_1, f_3;
    const id = VscTestIdModule_create(parseTestId(entry.TestId));
...
export function parseTestInfo(entry) {
    let x, x_1;
    const testIdStr = parseTestId(entry.TestId);
    ...
    switch (matchResult) {
        case 0: {
            ...
            patternInput = ((fields.length >= 2) ? [(x = item(0, fields), (x == null) ? undefined : x), (x_1 = (item(1, fields) | 0), (x_1 == null) ? undefined : x_1)] : [undefined, undefined]);
            break;
        }
        default:
...
export function parseFreshness(data) {
    const matchValue = parseDuCase(data.Freshness);
    let matchResult;
    if (matchValue != null) {
        switch (matchValue) {
...
export function parseResultsBatch(data) {
    let x;
    return defaultArg(map((entries_1) => {
        let x_1;
        const freshness = parseFreshness(data);
        const entryArray = defaultArg(map((value) => value, (x_1 = entries_1, (x_1 == null) ? undefined : some(x_1))), []);
    ...
    }, (x = data.Entries, (x == null) ? undefined : some(x))), empty());
}

export class LiveTestingCallbacks extends Record {
    constructor(OnStateChange, OnSummaryUpdate, OnStatusRefresh, OnBindingsUpdate, OnPipelineTraceUpdate) {
        super();
...
export class LiveTestingListener extends Record {
    constructor(State, Summary, Bindings, PipelineTrace, Dispose) {
        super();
        this.State = State;
...
export function start(port, callbacks) {
    ...
    return new LiveTestingListener(() => state, () => VscLiveTestStateModule_summary(state), () => bindings, () => pipelineTrace, () => {
        disposable.dispose();
    });
}
