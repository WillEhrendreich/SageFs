...
It runs on Windows, macOS, and Linux. SageFs itself installs with the .NET 10 SDK. Your *projects* aren't stuck on that version though: each session's host is built with your project's own SDK and runs on the runtime your project actually targets, so a .NET 11 project runs on .NET 11 even though SageFs itself is a .NET 10 tool.
