module SageFs.Middleware.ComputationExpression
...
let rewriteParsedExpr (parsed: Oak) =
  ...
  and mapClauses clauses =
    clauses
    |> List.map (fun (x: MatchClauseNode) -> MatchClauseNode(x.Bar, x.Pattern, x.WhenExpr, x.Arrow, mapExpr x.BodyExpr, x.Range))

  and getLeadingKeywordText (node: MultipleTextsNode) =
    node.Content 
    |> List.tryHead 
    |> Option.map (fun n -> n.Text)
    |> Option.defaultValue ""

  and mapStatement stmt =
    match stmt with
    | ComputationExpressionStatement.BindingStatement e when getLeadingKeywordText e.LeadingKeyword = "let!" ->
      ComputationExpressionStatement.BindingStatement(
        BindingNode(
          e.XmlDoc,
          e.Attributes,
          MultipleTextsNode([SingleTextNode("let", e.Range)], e.Range),
          e.IsMutable,
          e.Inline,
          e.Accessibility,
          e.FunctionName,
          e.GenericTypeParameters,
          e.Parameters,
          e.ReturnType,
          e.Equals,
          wrapInCompExpr e.Range e.Expr,
          None,
          e.Range
        )
      )
    | ComputationExpressionStatement.BindingStatement e when getLeadingKeywordText e.LeadingKeyword = "use!" ->
      ComputationExpressionStatement.BindingStatement(
        BindingNode(
          e.XmlDoc,
          e.Attributes,
          MultipleTextsNode([SingleTextNode("use", e.Range)], e.Range),
          e.IsMutable,
          e.Inline,
          e.Accessibility,
          e.FunctionName,
    ...
    | ComputationExpressionStatement.BindingStatement e when getLeadingKeywordText e.LeadingKeyword = "and!" ->
      ComputationExpressionStatement.BindingStatement(
        BindingNode(
          e.XmlDoc,
          e.Attributes,
